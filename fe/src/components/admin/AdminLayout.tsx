...
const AdminLayout = () => {
  ...
  // Mặc định false để không bị bật sidebar che màn hình trên mobile lúc mới vào
  ...
  // 2. Trạm kiểm soát Axios (Lắng nghe lỗi 401 toàn cục)
  // Nếu F5 mất token hoặc token hết hạn, tự động đăng xuất mà không báo lỗi khó hiểu
  useEffect(() => {
    const interceptor = axios.interceptors.response.use(
      (response) => response,
      (error) => {
        if (error.response && error.response.status === 401) {
          // Token hết hạn -> Xóa data -> Đá ra login luôn
  ...
  const handleLogout = async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      // Lỗi 401 kệ nó, không hiển thị lỗi đỏ
      console.log("Token đã mất hoặc hết hạn ở backend");
    } finally {
      // Bất chấp thành công hay thất bại, ép xóa user và về Login
      dispatch(setUser(null));
      navigate("/login");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col min-h-screen lg:pl-64 transition-all duration-300 w-full">
        {/* pt-20 để đẩy content xuống không bị lấp bởi Mobile Header, lg:pt-0 để bỏ trên desktop */}
